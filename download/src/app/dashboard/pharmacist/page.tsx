...
import { Pill, LogOut, Users } from 'lucide-react'; // Replaced FileText with Users

export default function PharmacistDashboardPage() {
  const auth = useAuth();
  const { currentTranslations } = useLanguage();
  const T = currentTranslations.pharmacistDashboard;
      ...
      <Card className="shadow-lg">
        <CardHeader>
          <CardTitle className="font-headline text-3xl text-primary flex items-center">
            <Pill className="mr-2 h-8 w-8" /> {T?.pageTitle || `Pharmacist Dashboard - Welcome, ${auth.user.name}!`}
          </CardTitle>
          <CardDescription>{T?.pageDescription || 'Manage prescriptions and pharmacy-related tasks.'}</CardDescription>
        </CardHeader>
        <CardContent className="grid md:grid-cols-1 gap-6">
          <DashboardActionCard
            title={T?.viewPatientRecordsTitle || "View Patient Records"}
            description={T?.viewPatientRecordsDescription || "Access patient files to view shared prescriptions."}
            link="/dashboard/pharmacist/patient-list"
